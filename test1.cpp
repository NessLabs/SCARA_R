...
// J1 homing tolerance using encoder only for zeroing/reference
#define J1_HOME_TOL       2.0f
...
// ─── Control loop — J1 and J2 both in open loop like steppers ─────────
void controlLoop(void* arg)
{
    if (g_estop) return;

    bool j1Done = motorJ1.atTarget();
    bool j2Done = motorJ2.atTarget();

    // only cut power when both stepper axes are done
    if (j1Done && j2Done)
    {
...
{
    motorJ1.setTarget(deg);

    TickType_t start = xTaskGetTickCount();
    while (!motorJ1.atTarget())
    {
    ...
    while (true)
    {
        if (xQueueReceive(cmdQueue, &cmd, pdMS_TO_TICKS(10)))
        {
            ...
            else if (strcmp(cmd.cmd, "pick") == 0)
            {
                ...
                strncpy(pendingCmd.pieceType, cmd.axis, sizeof(pendingCmd.pieceType) - 1);
                pendingCmd.pieceType[sizeof(pendingCmd.pieceType) - 1] = '\0';
            ...
            if (c == '\n' || c == '\r')
            {
                if (idx > 0)
                {
                    ...
                    if (sscanf(buf, "%f,%f,%15s", &x, &y, type) >= 2)
                    {
                        ...
                        pendingCmd.y   = y;
                        pendingCmd.phi = 0.0f;
                        strncpy(pendingCmd.pieceType, type, sizeof(pendingCmd.pieceType) - 1);
                        pendingCmd.pieceType[sizeof(pendingCmd.pieceType) - 1] = '\0';
                        pendingCmd.ready = true;
